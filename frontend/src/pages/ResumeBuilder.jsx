...
const ResumeBuilder = () => {
 ...
 const [resumeData, setResumeData] = useState({
   ...
   const [activeSectionIndex, setActiveSectionIndex] = useState(() => {
    return parseInt(localStorage.getItem(`section_${resumeId}`)) || 0
    }); 
  ...
  const saveResume = async () => {
    try {
      ...
      const {data} = await api.put('/api/resumes/update', formData, {headers: {Authorization: `Bearer ${token}`}})  // this line was missing
  ...
  />    
                ...
                <div className='flex items-center'>
                  {activeSectionIndex !== 0 && (
                    <button onClick={() => {const newIndex = Math.max(activeSectionIndex - 1, 0); setActiveSectionIndex(newIndex); localStorage.setItem(`section_${resumeId}`, newIndex)}} className='flex items-center gap-1 p-3 rounded-lg text-sm font-medium text-gray-600 hover:bg-gray-50 transition-all' disabled={activeSectionIndex === 0}> 
                      <ChevronLeft className='size-4'/> previous 
                    </button>
                  )}
                  <button onClick={() => {const newIndex = Math.min(activeSectionIndex + 1, sections.length - 1); setActiveSectionIndex(newIndex); localStorage.setItem(`section_${resumeId}`, newIndex)}} className={`flex items-center gap-1 p-3 rounded-lg text-sm font-medium text-gray-600 hover:bg-gray-50 transition-all ${activeSectionIndex === sections.length - 1 && 'opacity-50'}`} 
